...
FRAME_LEN = 640  # Byte
MSP_SUCCESS = 0  # 错误返回码，默认为没有错误
# 返回结果状态
MSP_AUDIO_SAMPLE_FIRST = 1
MSP_AUDIO_SAMPLE_CONTINUE = 2  # 音频状态
MSP_AUDIO_SAMPLE_LAST = 4
MSP_REC_STATUS_COMPLETE = 5
# 调用动态链接库
dll = cdll.LoadLibrary(r'..\sources\Windows_iat1218_tts_online1218_5ba9b76f\bin\msc_x64.dll')
# 登录参数设置
login_params = b"appid = 5ba9b76f, work_dir = Windows_iat1218_tts_online1218_5ba9b76f."
...
class Msp:
    ...
    def isr(self, audiofile, session_begin_params):
        ...
        # 创建语音识别会话，返回的是一个char指针作为sessionID以供后续接口使用，在Python里面要用ctypes.c_voidp类型
...
if __name__ == '__main__':
    path = r"C:\Users\pengfy\PycharmProjects\smart-audio\record_files\2019-07-30-11-52-51.wav"
    XF_text(path, 16000)